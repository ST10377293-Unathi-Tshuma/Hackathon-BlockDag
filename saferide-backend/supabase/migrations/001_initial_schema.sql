...
CREATE POLICY "Users can view their own user profile" ON user_profiles FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can update their own user profile" ON user_profiles FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own user profile" ON user_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Vehicle info policies
CREATE POLICY "Drivers can view vehicle info" ON vehicle_info FOR SELECT USING (
    EXISTS (SELECT 1 FROM drivers WHERE drivers.id = auth.uid() AND drivers.vehicle_id = vehicle_info.id)
);
...
-- Drivers policies
CREATE POLICY "Drivers can view their own profile" ON drivers FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Drivers can update their own profile" ON drivers FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Allow driver registration" ON drivers FOR INSERT WITH CHECK (auth.uid() = id);
...
CREATE POLICY "Drivers can view their own verification" ON driver_verification FOR SELECT USING (auth.uid() = driver_id);
CREATE POLICY "Drivers can create their own verification" ON driver_verification FOR INSERT WITH CHECK (auth.uid() = driver_id);
CREATE POLICY "Drivers can update their own verification" ON driver_verification FOR UPDATE USING (auth.uid() = driver_id);
...
CREATE POLICY "Users can view their own emergency incidents" ON emergency_incidents FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own emergency incidents" ON emergency_incidents FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own emergency incidents" ON emergency_incidents FOR UPDATE USING (auth.uid() = user_id);

-- Emergency contacts policies
CREATE POLICY "Users can view their own emergency contacts" ON emergency_contacts FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own emergency contacts" ON emergency_contacts FOR INSERT WITH CHECK (auth.uid() = user_id);
...
('Toyota', 'Camry', 2022, 'White', 'ABC123', 'sedan', 4),
('Honda', 'Civic', 2021, 'Black', 'XYZ789', 'sedan', 4),
('Tesla', 'Model 3', 2023, 'Blue', 'EV001', 'electric', 4);
